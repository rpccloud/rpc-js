...
describe("ieee754 tests", () => {
  test("Ieee754_read_write", () => {
    ...
    for (let i: number = 0; i < 1000; i++) {
      for (let j: number = 36; j < 58; j++) {
        let v: number = Math.random();
    ...
    // boundary numbers
    for (let v of [
      NaN, Number.POSITIVE_INFINITY, Number.NEGATIVE_INFINITY, Number.EPSILON,
      0, Number.MAX_SAFE_INTEGER, Number.MIN_SAFE_INTEGER, Number.MAX_VALUE,
    ...
    let v1: number = 2341325345.23423;
